        ...
        <div className="space-y-3">
          {filteredPosts.length === 0 ? (
            ...
            filteredPosts.map((post) => {
              ...
              return (
                <Card key={post.id} className="hover-scale group border-primary/12 bg-card/70 backdrop-blur-sm shadow-sm rounded-2xl overflow-hidden transition-all hover:shadow-md hover:border-primary/25">
                  <div className={`h-1.5 ${isLost ? "bg-gradient-to-r from-destructive/70 to-destructive/30" : "bg-gradient-to-r from-primary/70 to-primary/30"}`} />
                  <CardHeader className="pb-2 px-4 pt-4">
                    <div className="flex items-center gap-2.5">
                      <Badge className={`text-[0.65rem] font-bold px-2.5 py-1 ${
                        isLost ? "bg-destructive/10 border-destructive/30 text-destructive" : "bg-primary/10 border-primary/30 text-primary"
                      }`}>
                        {isLost ? "🔴 LOST" : "🟢 FOUND"}
                      </Badge>
                      <CardTitle className="text-sm sm:text-base font-bold text-foreground truncate">{post.title}</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3 text-xs px-4 pb-4">
                    <div className="flex gap-3">
                      <div className={`h-12 w-12 flex-shrink-0 rounded-xl flex items-center justify-center ${
                        isLost ? "bg-gradient-to-br from-destructive/10 to-destructive/5 border border-destructive/20" : "bg-gradient-to-br from-primary/15 to-primary/5 border border-primary/20"
                      }`}>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-accent/20 border border-accent-foreground/15 px-2.5 py-1 text-[0.7rem] font-medium text-accent-foreground">
                        <MapPin className="h-3 w-3" /> {post.location}
                      </span>
                      {post.approximate_time && (
                        <span className="inline-flex items-center gap-1 rounded-full bg-primary/8 border border-primary/15 px-2.5 py-1 text-[0.7rem] font-medium text-primary">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      <Button
                        ...
                        className="h-7 rounded-full text-[0.65rem] px-3"
                      ...
                      {isOwner && (
                        <Button size="sm" variant="outline" className="h-7 rounded-full text-[0.65rem] px-3 border-destructive/20 text-destructive hover:bg-destructive/5" onClick={() => handleRemovePost(post.id)}>
                          Remove
                        </Button>
                      )}
                    </div>
